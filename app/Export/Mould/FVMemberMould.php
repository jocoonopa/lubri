...
class FVMemberMould extends FVMould
{
    public function getRow(array $member)
    {
        ...
        $arr = [
            ...
            $this->convertDate($this->transfer(array_get($member, '生日')), 'Ymd'),
            ...
            $this->convertDate(substr($this->transfer(array_get($member, '首次購物日')), 0, 19)), //2011-01-01 00:00:00 => length = 19
            $this->transfer(array_get($member, '最後購物金額')),
            $this->convertDate(substr($this->transfer(array_get($member, '最後購物日')), 0, 19)), 
            $this->transfer(array_get($member, '累積購物金額')),
            $this->transfer(array_get($member, '累積紅利點數')), 
            $this->transfer(array_get($member, '輔翼會員參數')),
            $this->convertDate($this->transfer(array_get($hd, 'period')), 'Ymd'),
